...
public class DobroPost {
	...
	public void formatText() {
		if (message != null)
			this.formattedText = DobroFormatter.getFormatted(message,
					getBoardName(), getLastContext(), this);
		else if (message_html != null)
			try {
				String source = message_html;
				source = Pattern
						.compile(
								"<a href=\"/\\w+?/res/\\d+?.xhtml#i\\d+?\" onmouseover=\"ShowRefPost\\(event,'(\\w+?)', (\\d+?), (\\d+?)\\)\" "
										+ "onclick=\"Highlight\\(event, '\\d+?'\\)\">(&gt;&gt;.*?)</a>",
								Pattern.DOTALL).matcher(source)
						.replaceAll("<link_$1_$2_$3>$4</link_$1_$2_$3>");
				// source =
				// Pattern.compile("<span class=\"spoiler\">(.*?)</span>",
				// Pattern.DOTALL).matcher(source).replaceAll("<span>$1</span>");
				source = Pattern
						.compile("<div class=\"spoiler\">(.*?)</div>",
								Pattern.DOTALL)
						.matcher(source)
						.replaceAll(
								"<br/><span class=\"spoiler\">$1</span><br/>");
				Matcher m = Pattern.compile("<pre>(.*?)</pre>", Pattern.DOTALL)
						.matcher(source);
				while (m.find())
					source = source.replace(m.group(),
							m.group().replaceAll("\n", "<br/>"));
				final DobroHtmlTagHandler dph = new DobroHtmlTagHandler(
						getLastContext(), this);
				this.formattedText = DobroHtml.fromHtml(source, null, dph);
				if (this.formattedText.length() >= 2)
					this.formattedText = this.formattedText.subSequence(0,
							this.formattedText.length() - 1);
	...
	public void checkSpells() {
		String db_comment = DobroApplication.getApplicationStatic()
				.getHiddenPosts()
				.getComment(getBoardName() + "/" + getDisplay_id());
		if (db_comment != null) {
