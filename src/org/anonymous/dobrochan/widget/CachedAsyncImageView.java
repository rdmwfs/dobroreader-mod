...
public class CachedAsyncImageView extends ImageView implements OnClickListener,
	...
	private class BitmapLoader extends AsyncTask<String, Void, Bitmap> {
		...
		protected void onPostExecute(Bitmap result) {
			try {
				if (!TextUtils.equals(currentUrl, mCachedUrl))
					return;
				if (result == null) {
					ImageLoader.getInstance().displayImage(mCachedUrl,
							CachedAsyncImageView.this,
		...
		protected Bitmap doInBackground(String... params) {
			...
			try {
				Bitmap bmp = DobroApplication.getApplicationStatic()
						.getNetwork().memory_cache.get(mCachedUrl);
				if (bmp != null)
					return bmp;
				File cached = DobroApplication.getApplicationStatic()
						.getNetwork().disc_cache.get(mCachedUrl);
	...
	public void setUrl(String url) {
		...
		if (mBitmap != null && url != null && mCachedUrl != null
				&& url.equals(mCachedUrl)) {
			return;
		}
		mCachedUrl = url;
		if (mBitmap != null)
		...
		urlLoaded = DobroHelper.checkNetworkForPictures(getContext())
				|| mForceLoad
				|| DobroApplication.getApplicationStatic().getNetwork().memory_cache
						.get(mCachedUrl) != null
				|| DobroApplication.getApplicationStatic().getNetwork().disc_cache
						.get(mCachedUrl).exists();
	...
	public void downloadImage(boolean open, String prefix, int size) {
		...
		File pic = quick_search ? getFileRecQuick(picsSearchDir, fname)
				: getFileRec(picsSearchDir, fname);
		if (pic != null) {
			if (open)
		...
		if (internal_viewer
				&& open
				&& (urlTag.endsWith("jpg") || urlTag.endsWith("jpeg")
						|| urlTag.endsWith("png") || urlTag.endsWith("bmp") || urlTag
							.endsWith("gif"))) {
			...
			if (size != -1)
				i.putExtra("sample", String.format(DobroConstants.RESIZER_URL,
						Uri.encode(DobroConstants.HOST + urlTag, ":/"), size,
						size));
			else if ((sizeW > max_size || sizeH > max_size)
					&& (urlTag.endsWith("png") || urlTag.endsWith("jpeg") || urlTag
							.endsWith("jpg")) && max_size > 0)
				i.putExtra("sample", String.format(DobroConstants.RESIZER_URL,
	...
	public void onCreateContextMenu(ContextMenu menu, View v,
		...
		menu.add("Копировать ссылку").setOnMenuItemClickListener(
				new OnMenuItemClickListener() {
					...
					@Override
					public boolean onMenuItemClick(MenuItem item) {
						String link = DobroConstants.HOST + urlTag;
						...
						Toast.makeText(getContext(), link, Toast.LENGTH_LONG)
								.show();
						return true;
					}
				});
		menu.add("Расшарить").setOnMenuItemClickListener(
				new OnMenuItemClickListener() {
					@Override
					public boolean onMenuItemClick(MenuItem item) {
						...
						intent.putExtra(Intent.EXTRA_TEXT, DobroConstants.HOST
								+ urlTag);
		...
		menu.add("Сведения").setOnMenuItemClickListener(
				new OnMenuItemClickListener() {
					@Override
					public boolean onMenuItemClick(MenuItem item) {
						...
						edit.setText(mInfo);
						Dialog d = new Dialog(getContext(), DobroHelper
		...
		if (sizeH <= 0 && sizeW <= 0)
			return;
		if (!(urlTag.endsWith("jpg") || urlTag.endsWith("jpeg")
				|| urlTag.endsWith("png") || urlTag.endsWith("gif")))
			return;
		if (sizeH > 800 && sizeW > 800 && !urlTag.endsWith("gif"))
			menu.add("[800px] Уменьшенная").setOnMenuItemClickListener(
					new OnMenuItemClickListener() {
						@Override
						public boolean onMenuItemClick(MenuItem item) {
		...
		if (Math.max(sizeH, sizeW) / 5 <= 4000 && !urlTag.endsWith("gif"))
			menu.add("[20%] Уменьшенная").setOnMenuItemClickListener(
					new OnMenuItemClickListener() {
						@Override
						public boolean onMenuItemClick(MenuItem item) {
							downloadImage(true, "20_",
									Math.max(sizeH, sizeW) / 5);
							return true;
						}
					});
		if (Math.max(sizeH, sizeW) / 5 <= 4000 && !urlTag.endsWith("gif"))
			menu.add("[50%] Уменьшенная").setOnMenuItemClickListener(
					new OnMenuItemClickListener() {
						@Override
						public boolean onMenuItemClick(MenuItem item) {
							downloadImage(true, "50_",
									Math.max(sizeH, sizeW) / 2);
							return true;
						}
					});
		menu.add("Google Search").setOnMenuItemClickListener(
				new OnMenuItemClickListener() {

					@Override
					public boolean onMenuItemClick(MenuItem item) {
						String url = "http://www.google.com/searchbyimage?image_url="
								+ DobroHelper.formatUri("http://dobrochan.ru/"
										+ urlTag);
		...
		menu.add("IQDB Search").setOnMenuItemClickListener(
				new OnMenuItemClickListener() {

					@Override
					public boolean onMenuItemClick(MenuItem item) {
						String url = "http://iqdb.org/?url="
								+ DobroHelper.formatUri("http://dobrochan.ru/"
										+ urlTag);
						Intent i = new Intent(Intent.ACTION_VIEW);
						i.setData(Uri.parse(url));
		...
		menu.add("TinEye Search").setOnMenuItemClickListener(
				new OnMenuItemClickListener() {

					@Override
					public boolean onMenuItemClick(MenuItem item) {
						String url = "http://tineye.com/search/?url="
								+ DobroHelper.formatUri("http://dobrochan.ru/"
										+ urlTag);
						Intent i = new Intent(Intent.ACTION_VIEW);
						i.setData(Uri.parse(url));
	...
	private static Pattern probablyNoFolder = Pattern
			.compile("^.+?\\.(jpg|png|gif|svg)");
	...
	private static File getFileRecQuick(File dir, String fname) {
		try {
			...
			for (int i = 0; i < children.length; i++) {
				...
				if (file == null
						&& !probablyNoFolder.matcher(children[i]).matches()) {
					file = new File(dir, children[i]);
					if (!file.isDirectory())
						file = null;
				}
				if (file != null
						&& (file = getFileRecQuick(file, fname)) != null)
					return file;
			}
		} catch (Exception e) {
		}
