...
//XXX
		...
		if (dobroitem.post.isOp() && !dobroitem.post.isIn_thread()) {
			...
			if (dobroitem.post.getThread() != null)
				intent.putExtra(ActionBarActivity.GD_ACTION_BAR_TITLE,
						dobroitem.post.getThread().getTitle());
			else
				intent.putExtra(ActionBarActivity.GD_ACTION_BAR_TITLE,
			 ...
			 * DobroNewPostActivity.class);
			 * intent.putExtra(DobroConstants.BOARD,
			 * dobroitem.post.getBoardName());
			 * intent.putExtra(DobroConstants.THREAD,
			 * dobroitem.post.getThreadDisplay_id());
			 * intent.putExtra(DobroConstants.POST,
			 * dobroitem.post.getDisplay_id()); intent.putExtra(
			 * GDActivity.GD_ACTION_BAR_TITLE, String.format("Ответ в >>%s/%s",
			 * dobroitem.post.getBoardName(),
			 * dobroitem.post.getThreadDisplay_id()));
			 * getContext().startActivity(intent);
			 */
	...
	public void setObject(Item item) {
		 ...
		 * refsView.setClickable(false); refsView.setFocusable(false);
		...
		getContext().getTheme().resolveAttribute(R.attr.dcPicrelatedColor,
				backgroundRef, true);
		...
		if (dobroitem.post.getFiles().length > 0) {
			for (DobroFile file : dobroitem.post.getFiles()) {
				...
				String fname1 = file.getThumb().substring(
						file.getThumb().lastIndexOf("/") + 1);
				int e = fname1.lastIndexOf("s.");
				if (e > 0)
					fname1 = fname1.substring(0, e)
							+ file.getSrc().substring(
									file.getSrc().lastIndexOf("."));
				String fname2 = file.getSrc().substring(
						file.getSrc().lastIndexOf("/") + 1);
				...
				if (show_info) {
					...
					if (file.getMetadata() != null
						...
						text += ", " + String.valueOf(file.getMetadata().width)
								+ "x"
				...
				int w = Math
						.min(Math.round((h / file.getThumb_height())
								* file.getThumb_width()), file.getThumb_width());
				imageView.setLayoutParams(new LayoutParams(w + 4,
						mImageHeight + 4));
				if (show_info) {
		...
		if (prefs.getBoolean("youtube", true)) {
			...
			if (cast_ok) {
				...
				for (URLSpan span : spans) {
					...
					Pattern p = Pattern
							.compile("(youtube\\.com/watch\\?v=|youtu\\.be/)(\\S{11})");
					Matcher m = p.matcher(url);
					if (!m.find())
						continue;
					String yt_id = m.group(2);
					String text = s.subSequence(s.getSpanStart(span),
							s.getSpanEnd(span)).toString();
					CachedAsyncImageView imageView = new CachedAsyncImageView(
							getContext());
					imageView
							.setLayoutParams(new LayoutParams(
									(int) ((mImageHeight + 4) / 0.75),
									mImageHeight + 4));
					...
					imageView.setUrl("http://img.youtube.com/vi/" + yt_id
							+ "/0.jpg");

					imageView.setBackgroundColor(backgroundColor);
					imageView.setScaleType(ScaleType.CENTER);

					VerticalTextView textView = new VerticalTextView(
							getContext(), null);
					textView.setGravity(Gravity.BOTTOM);
					textView.setTextSize(TypedValue.COMPLEX_UNIT_SP, 12);
					textView.setText(text);
		...
		metadataRightView.setText(getContext().getString(R.string.post_date,
		...
		if (!refs.isEmpty()) {
			 ...
			 * s.setSpan(new StyleSpan(Typeface.BOLD_ITALIC), 0, seq.length(),
			 * 0); s.append(" "); for (String ref : refs) { int start =
			 * s.length(); s.append(ref + ", "); s.setSpan( new
			 * DobroLinkSpan(dobroitem.post.getBoardName(), ref .substring(2),
			 * getContext(), null), start, start + ref.length(), 0); }
	...
	public void onMenuOpenClicked() {
		...
		if (dobroitem.post.getThreadDisplay_id() == null)
			intent.putExtra(DobroConstants.THREAD_ID,
					dobroitem.post.getThread_id());
		intent.putExtra(DobroConstants.BOARD, dobroitem.post.getBoardName());
		intent.putExtra(DobroConstants.THREAD, dobroitem.post
				.getThreadDisplay_id() == null ? dobroitem.post.getDisplay_id()
				: dobroitem.post.getThreadDisplay_id());
		if (dobroitem.post.getThread() != null)
			intent.putExtra(ActionBarActivity.GD_ACTION_BAR_TITLE,
					dobroitem.post.getThread().getTitle());
	...
	public void onMenuAnswerClicked() {
		...
		intent.putExtra(ActionBarActivity.GD_ACTION_BAR_TITLE, String.format(
				"Ответ в >>%s/%s", dobroitem.post.getBoardName(),
				dobroitem.post.getThreadDisplay_id()));
	...
	public void onMenuCopyClicked() {
		String text = dobroitem.post.getMessage() == null ? dobroitem.post
				.getFormattedText().toString() : dobroitem.post.getMessage();
		/*
		 * Intent i = new Intent(getContext(), CopyPasteActivity.class);
		 * i.putExtra("text", ); i.putExtra(GDActivity.GD_ACTION_BAR_TITLE,
		 * "Скопируйте текст"); getContext().startActivity(i);
		 */
		View v = null;
	...
	private class ThreadManager extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			if (to_del)
				DobroNetwork.getInstance().delPost(board,
						dobroitem.post.getThread_id(),
	 ...
	 *            number to format
	 * @param powerOfTen
	 *            use 1kB = 1000B if "true" or use 1KiB = 1024B if "false
	 * @return
	 */
	public static String humanReadableByteCount(long bytes, boolean powerOfTen) {
		int unit = powerOfTen ? 1000 : 1024;
		if (bytes < unit)
			return bytes + " B";
		int exp = (int) (Math.log(bytes) / Math.log(unit));
		String pre;
		try {
			pre = (powerOfTen ? "kMGTPEZY" : "KMGTPEZY").charAt(exp - 1)
					+ (powerOfTen ? "" : "i");
		} catch (IndexOutOfBoundsException e) {
			return "ДОХУЯ";
		}
	...
	public void onCreateContextMenu(ContextMenu menu, View v,
		...
		if (show_open)
			menu.add("Открыть в фоне").setIcon(R.drawable.chat_32x32)
					.setOnMenuItemClickListener(new OnMenuItemClickListener() {
						@Override
						public boolean onMenuItemClick(MenuItem arg0) {
							DobroTabsHolder.getInstance()
									.addTab(board,
											thread,
											null,
											dobroitem.post.getThread()
													.getTitle(), null);
							new Thread(new Runnable() {
								@Override
								public void run() {
									DobroNetwork.getInstance().getThreadJson(
											String.format("%s/%s", board,
													thread), null, null, false);
								}
							}).start();
							return true;
						}
		...
		menu.add("Копировать ссылку").setOnMenuItemClickListener(
				new OnMenuItemClickListener() {
					@SuppressWarnings("deprecation")
					@SuppressLint("NewApi")
					@Override
					public boolean onMenuItemClick(MenuItem item) {
						String postLink = DobroConstants.HOST.concat(board)
								.concat("/res/").concat(thread)
								.concat(".xhtml#i")
								.concat(dobroitem.post.getDisplay_id());
						...
						Toast.makeText(getContext(), postLink,
								Toast.LENGTH_SHORT).show();
						return true;
					}
				});
		if (!hidden)
			menu.add("Скрыть пост").setOnMenuItemClickListener(
					new OnMenuItemClickListener() {
						@Override
						public boolean onMenuItemClick(MenuItem item) {
							DobroApplication
			...
			menu.add("Раскрыть пост").setOnMenuItemClickListener(
					new OnMenuItemClickListener() {
						@Override
						public boolean onMenuItemClick(MenuItem item) {
							DobroApplication
