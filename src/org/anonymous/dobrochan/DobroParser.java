...
public class DobroParser extends Object {
	...
	public void parceError(JsonObject err) {
		try {
			...
			DobroApplication app = DobroApplication.getApplicationStatic();
			app.showToast(String.format("Error %s: %s",
					err.getAsJsonPrimitive("code").getAsString(), err
							.getAsJsonPrimitive("message").getAsString()), 2);
		} catch (JsonSyntaxException e) {
		}
	}
		...
		try {
			...
			for (DobroFile attach : p.getFiles()) {
				...
				boolean urlLoaded = DobroHelper.checkNetworkForPictures(c)
						...
						&& !DobroApplication.getApplicationStatic()
								.getNetwork().disc_cache.get(
								DobroHelper.formatUri(attach.getThumb()))
								.exists();
				Rating rat = attach.getRat();
				if (DobroHelper.checkRating(c, rat))
					urlLoaded = false;
				if (urlLoaded) {
					// DobroNetwork.getInstance().addPendingUrl(null,
					// attach.getThumb());
				}
			}
		} catch (JsonSyntaxException e) {
		...
		try {
			...
			for (DobroThread t : sess.getThreads()) {
				if (!t.getLevel().equals("bookmarked"))
					continue;
				DobroThread info = DobroNetwork.getInstance()
						.getThreadInfoJson(t.getThread_id());
			...
			if (threads2del.size() > 0) {
				ArrayList<DobroThread> temp = new ArrayList<DobroThread>(
						Arrays.asList(sess.getThreads()));
		...
		try {
			...
			if (t.getPosts() != null) {
				for (DobroPost p : t.getPosts()) {
					p.setThread(t);
					p.formatText();
					// p.setFormattedText(new
					// SpannableString(p.getMessage()==null?p.getMessageHtml():p.getMessage()));
				}
				createReflinks(t);
			} else
	...
	private void createReflinks(DobroThread t) {
		...
		for (DobroPost p : t.getPosts()) {
			Pattern link_pattern = Pattern.compile(">>(?:/)?(\\d+)");
