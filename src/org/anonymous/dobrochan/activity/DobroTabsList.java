...
public class DobroTabsList extends GDListActivity {
	...
	protected void onResume() {
		super.onResume();
		m_adapter.clear();
		for(String[] s : Reversed.reversed(DobroTabsHolder.getInstance().getTabs())) {
			String board = s[0];
			String thread = s[1];
			String title = s[3];
			String post = s[4];
			...
			if(board != null && post != null)
				item = new SubtitleItem(title, String.format(">>%s/%s", board, post));
			else if(board != null && thread != null)
				item = new SubtitleItem(title, String.format(">>%s/%s", board, thread));
			else if(board != null) {
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		if(tag == null) {
			...
			i.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK); //TODO: API < 11 ?
			startActivity(i);
		} else {
			String board = tag[0];
			String thread = tag[1];
			...
			if(board != null && post != null) {
				...
				i.putExtra(GD_ACTION_BAR_TITLE, String.format(">>%s/%s", board, post));
				i.setFlags(flags);
				startActivity(i);
			}
			else if(board != null && thread != null) {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(i != null) {
			...
			DobroTabsHolder.getInstance().addTab(board, thread, scroll, text, post);
		}
	}
}
