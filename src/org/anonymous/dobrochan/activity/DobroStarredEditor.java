...
public class DobroStarredEditor extends GDListActivity {
	...
	private class ThreadListLoader extends AsyncTask<String, Void, DobroThread[] >
	{
		ProgressDialog dlg;
		@Override
...
//					subtitle += getString(R.string.unreaded_posts);
					subtitle += getString(R.string.faw_thread_edited,t.getLastModified());
					title = "*" + title;
				}
		...
		protected void onPreExecute() {
			super.onPreExecute();
			dlg = ProgressDialog.show(DobroStarredEditor.this, getString(R.string.loading), "", true, true, new OnCancelListener() {
		...
		protected DobroThread[] doInBackground(String... params) {
			...
			if(params != null && params.length > 0 && params[0] != "")
				result = DobroApplication.getApplicationStatic().getParser().parceThreads(params[0]);
			if(result != null)
				return result;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		refresh_item = (LoaderActionBarItem)addActionBarItem(ActionBarItem.Type.Refresh, AB_REFRESH);
		...
		if(atask == null)
			atask = (ThreadListLoader)new ThreadListLoader().execute(getIntent().hasExtra(DobroConstants.FAVS_DUMP)?getIntent().getStringExtra(DobroConstants.FAVS_DUMP):"");
		NotificationManager mNotificationManager = (NotificationManager) getSystemService(Context.NOTIFICATION_SERVICE);
		mNotificationManager.cancel(DobroConstants.NOTIFY_ID);
