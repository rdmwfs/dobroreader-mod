...
public class DobroThreadActivity extends DobroPostsList {
	...
	public void onLowMemory() {
		Toast.makeText(this,
				"Недостаточно свободной памяти. Enlarge your heap.",
	...
	protected void onPause() {
		...
		String value = null;
		try {
			DobroPostItem dpi = (DobroPostItem) m_adapter.getItem(getListView()
	...
	protected void onResume() {
		...
		if (resume_autoupdate) {
			...
			future_task = executor.scheduleAtFixedRate(autoupdater_runnable, 5,
					autocheck_interval, TimeUnit.SECONDS);
		}
		super.onResume();
	}
	...
	public boolean onHandleActionBarItemClick(ActionBarItem item, int position) {
		if (position == -1) {
			...
			String value = null;
			try {
				DobroPostItem dpi = (DobroPostItem) m_adapter
						.getItem(getListView().getFirstVisiblePosition());
				value = dpi.post.getDisplay_id();
			} catch (Exception e) {
				e.printStackTrace();
			}
		...
		case AB_ADD:
			...
			i.putExtra(ActionBarActivity.GD_ACTION_BAR_TITLE,
					String.format("Ответ в >>%s/%s", board, thread));
		...
		case AB_WAKE:
			if (wl == null) {
				...
				Toast.makeText(this, "Отключение подсветки запрещено",
						Toast.LENGTH_SHORT).show();
			} else {
				wl.release();
				wl = null;
				item.setDrawable(R.drawable.moon_stroke_32x32);
				Toast.makeText(this, "Отключение подсветки разрешено",
						Toast.LENGTH_SHORT).show();
			}
			return true;
		case AB_REFRESH:
			if (future_task != null) {
				...
				Toast.makeText(this, "Автообновление отключено",
						Toast.LENGTH_SHORT).show();
			} else {
				try {
					executor.remove(autoupdater_runnable);
				...
				future_task = executor.scheduleAtFixedRate(
						autoupdater_runnable, 5, autocheck_interval,
						TimeUnit.SECONDS);
				item.setDrawable(R.drawable.loop_red_32x32);
				Toast.makeText(this, "Автообновление активировано",
						Toast.LENGTH_SHORT).show();
			}
			return true;
		default:
	...
	protected void onNewIntent(Intent intent) {
		...
		if (Intent.ACTION_VIEW.equals(intent.getAction())
				&& intent.hasExtra(DobroConstants.FAVS_DUMP)) {
	...
	private class ThreadLoader extends
		...
		protected DobroThread doInBackground(ThreadLoaderParams... params) {
			try {
				...
				/*
				 * try{ String title =
				 * getIntent().getStringExtra(GDActivity.GD_ACTION_BAR_TITLE);
				 * if(title.equalsIgnoreCase("Понитред")) new
				 ...
				 * h.post(new Runnable() {
				 * 
				 * @Override public void run() { tryJoke(); } }); } catch
				 * (Exception e) { } } }, 1000); super.onPreExecute(); } catch
				 * (Exception e) { e.printStackTrace(); }
				 */
				long tm = System.currentTimeMillis();
				json = DobroNetwork.getInstance().getThreadJson(adress,
						m_param.last_post, m_param.count, m_param.force);
				Log.d("TIME",
						"Network time: "
				...
				tm = System.currentTimeMillis();
				/*
				 * if (t!=null&& t.getTitle()!=null&& (Md5Util.md5(t.getTitle())
				 * .equalsIgnoreCase(DobroConstants.DCOTT))) t = null;
				 */
				if (t.isThreadModified() || t == null)
					return t;
				else {
					if (getIntent().getBooleanExtra(
							DobroConstants.DISABLE_UPDATE, false)) {
						t.setThreadModified(true);
						return t;
					} else {
						this.publishProgress(t);
						params[0].last_post = t.getPosts()[t.getPosts().length - 1]
								.getDisplay_id();
		...
		protected void onPostExecute(DobroThread result) {
			...
			if (result == null || result.getPosts().length == 0) {
				if (result == null) {
					Toast.makeText(DobroThreadActivity.this, "Ошибка",
							Toast.LENGTH_SHORT).show();
				}
			...
			nextPage.setTag(DobroConstants.UPDATE_TAG
					+ result.getPosts()[result.getPosts().length - 1]
							.getDisplay_id());
			...
			if (schrollTo != null) {
				...
				getListView().setSelection(
						postPositionMap.get(result.getPosts()[0]
								.getDisplay_id()));
			atask = null;
			Log.d("TIME",
					"Post exec time: "
			...
			tm = System.currentTimeMillis();
			if (m_param.beep && autocheck_sound)
				try {
					String snd_uri = DobroApplication
					...
					// r.play();
					mMediaPlayer.reset();
					mMediaPlayer.setDataSource(DobroThreadActivity.this,
							notification);
	...
	void updateProgressView(View v, ProgressItem item) {
		try {
			ProgressBar mProgressBar = (ProgressBar) v
					.findViewById(R.id.gd_progress_bar);
			mProgressBar.setVisibility(item.isInProgress ? View.VISIBLE
					: View.GONE);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		try {
			autocheck_interval = Integer.parseInt(prefs.getString(
				...
				DobroThreadActivity.this.runOnUiThread(new Runnable() {
					...
					public void run() {
						try {
							...
							if (!pi.isInProgress
									&& tag != null
									&& tag.startsWith(DobroConstants.UPDATE_TAG)) {
								...
								new ThreadLoader().execute(new ThreadLoaderParams(
										board, thread,
										tag.substring(DobroConstants.UPDATE_TAG
												.length()), count, true));
							}
						} catch (Exception e) {
							e.printStackTrace();
		...
		if (getIntent().hasExtra(DobroConstants.SCHROLL_TO))
			this.schrollTo = getIntent().getStringExtra(
					DobroConstants.SCHROLL_TO);
		else
			this.schrollTo = null;

		handleUrl();

		addActionBarItem(ActionBarItem.Type.Slideshow, AB_REFRESH).setDrawable(
				R.drawable.loop_32x32);
		addActionBarItem(ActionBarItem.Type.Eye, AB_WAKE).setDrawable(
				R.drawable.moon_stroke_32x32);
	...
	private void handleUrl() {
		...
		if (data != null) {
			Pattern p = Pattern
					.compile("dobrochan\\.(ru|org)/(.*)/res/(\\d+)\\.xhtml(#i.*)?");
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.copy_link: {
			String threadLink = DobroConstants.HOST + board + "/res/" + thread
					+ ".xhtml";
			if (Build.VERSION.SDK_INT < 11) {
				// XXX SDK_INT < 11
				android.text.ClipboardManager clipboard = (android.text.ClipboardManager) getSystemService(Context.CLIPBOARD_SERVICE);
				clipboard.setText(threadLink);
			} else {
				android.content.ClipboardManager clipboard = (android.content.ClipboardManager) getSystemService(Context.CLIPBOARD_SERVICE);
				clipboard.setPrimaryClip(ClipData
						.newPlainText(null, threadLink));
			}
			Toast.makeText(this, threadLink, Toast.LENGTH_SHORT).show();
			return true;
		}
		}
	...
	public void addPosts(DobroThread t) {
		if (t == null || t.getPosts() == null || postPositionMap == null
				|| postPositionMap.isEmpty())
			return;
		...
		for (DobroPost p : posts) {
			for (String id : p.getLinks()) {
				try {
					...
					if (pos == null)
						continue;
					DobroPostItem i = (DobroPostItem) this.getListAdapter()
							.getItem(pos);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (requestCode == DobroConstants.ANSWER_RESULT
				&& resultCode == Activity.RESULT_OK) {
			if (!PreferenceManager.getDefaultSharedPreferences(this)
					.getBoolean("update_after_posting", true))
				return;
