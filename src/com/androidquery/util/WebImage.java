...
public class WebImage extends WebViewClient {
	...
	private void fixMaxZoom() {
		/*
		 * try{ Class<?> webViewClass = wv.getClass(); Field mZoomManagerField =
		 * webViewClass.getDeclaredField("mZoomManager");
		 * mZoomManagerField.setAccessible(true); Object mZoomManagerInstance =
		 * mZoomManagerField.get(wv); Class<?> zoomManagerClass =
		 * Class.forName("android.webkit.ZoomManager"); Field
		 * mDefaultMaxZoomScaleField =
		 * zoomManagerClass.getDeclaredField("mDefaultMaxZoomScale");
	 ...
	 * 
	 * if(url.equals(wv.getTag(AQuery.TAG_URL))){ return; }
	 * 
	 ...
	 * 
	 * //state, "init", "fetch" wv.setTag(AQuery.TAG_LAYOUT, "init");
	 * 
	 * wv.setPictureListener(new PictureListener() {
	 * 
	 * @Override public void onNewPicture(WebView view, Picture picture) {
	 * 
	 * String url = (String) view.getTag(AQuery.TAG_URL); String state =
	 * (String) view.getTag(AQuery.TAG_LAYOUT);
	 * 
	 ...
	 * 
	 * if("init".equals(state)){ view.setTag(AQuery.TAG_LAYOUT, "fetch");
	 * //lastWidth = width; setupWebview(view, url); }else
	 * if("fetch".equals(state)){ view.setVisibility(View.VISIBLE);
	 ...
	 * 
	 * if(("fetch".equals(view.getTag(AQuery.TAG_LAYOUT)))){ if(progress !=
	 * null){ progress.setVisibility(View.GONE); } view.setWebViewClient(null);
	 * }
	 * 
	 * }
	 * 
	 ...
	 * 
	 * int width = wv.getWidth(); int height = wv.getHeight();
	 * 
	 * wv.setInitialScale(100);
	 * 
	 ...
	 * 
	 * View parent = (View) wv.getParent(); AQUtility.debug("parent",
	 * parent.getWidth() + "x" + parent.getHeight());
	 * 
	 * String html = getSource(wv.getContext()); html =
	 * html.replaceAll("@width", width + "").replaceAll("@height", height +
	 * "").replaceAll("@src", url);
	 * 
	 * String dim;
	 * 
	 ...
	 * 
	 * if(!landscape){ dim = "width:" + width + "px;"; }else{ dim = "height:" +
	 * height + "px;"; } html = html.replaceAll("@dim",
	 * dim).replaceAll("@scape", landscape + "");
	 * 
	 * //AQUtility.debug(html);
	 * 
	 * WebSettings ws = wv.getSettings(); ws.setSupportZoom(true);
